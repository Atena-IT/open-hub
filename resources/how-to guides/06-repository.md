...
Create an empty repository with [create_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_repo) and give it a name with the `repo_id` parameter. The `repo_id` is your namespace followed by the repository name: `username_or_org/repo_name`.
...
By default, [create_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_repo) creates a model repository. But you can use the `repo_type` parameter to specify another repository type. For example, if you want to create a dataset repository:
...
If you want to change the repository visibility at a later time, you can use the [update_repo_settings()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.update_repo_settings) function.

> [!TIP]
> If you are part of an organization with an Enterprise plan, you can create a repo in a specific resource group by passing `resource_group_id` as parameter to [create_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_repo). Resource groups are a security feature to control which members from your org can access a given resource. You can get the resource group ID by copying it from your org settings page url on the Hub (e.g. `"https://huggingface.co/organizations/huggingface/settings/resource-groups/66670e5163145ca562cb1988"` => `"66670e5163145ca562cb1988"`). For more details about resource group, check out this [guide](https://huggingface.co/docs/hub/en/security-resource-groups).

### Delete a repository

Delete a repository with [delete_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.delete_repo). Make sure you want to delete a repository because this is an irreversible process!
...
This is possible using the [duplicate_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.duplicate_repo) method. It will duplicate the whole repository, preserving the full git history.
...
You can create new branch and tags using [create_branch()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_branch) and [create_tag()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.create_tag):
...
You can use the [delete_branch()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.delete_branch) and [delete_tag()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.delete_tag) functions in the same way to delete a branch or a tag, or `hf repos branch delete` and `hf repos tag delete` respectively in CLI.

### List all branches and tags

You can also list the existing git refs from a repository using [list_repo_refs()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.list_repo_refs):
...
You can set a repo as gated using [update_repo_settings()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.update_repo_settings):
...
You can rename your repository on the Hub using [move_repo()](/docs/huggingface_hub/v1.6.0/en/package_reference/hf_api#huggingface_hub.HfApi.move_repo). Using this method, you can also move the repo from a user to
