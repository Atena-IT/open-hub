...
A `thiserror`-derived enum with 12 variants covering the full error space of the server. Each variant maps to one or more HTTP status codes in the `IntoResponse` implementation.
